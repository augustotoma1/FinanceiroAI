...
class TestContaAzulServiceInitialization:
    ...
    def test_initialization_success(self):
        ...
        assert service.base_url == settings.CONTA_AZUL_API_BASE_URL
        assert service.auth_url == f"{service.base_url}{service.AUTH_ENDPOINT}"
        assert service.token_url == f"{service.base_url}{service.TOKEN_ENDPOINT}"
...
class TestGetCustomers:
    ...
    async def test_get_customers_success(self, service):
        ...
        with patch.object(service, 'make_api_request', new=AsyncMock(return_value={"data": mock_customers})):
    ...
    async def test_get_customers_with_search(self, service):
        """Test customer retrieval with search parameter"""
        ...
        with patch.object(service, 'make_api_request', new=AsyncMock(return_value={"data": mock_customers})):
            ...
            assert call_kwargs["params"]["search"] == "João"

    @pytest.mark.asyncio
    async def test_get_customers_with_custom_limit(self, service):
        """Test customer retrieval with custom limit"""
    ...
    async def test_get_customers_empty_result(self, service):
        """Test customer retrieval with empty result"""
        with patch.object(service, 'make_api_request', new=AsyncMock(return_value={"data": []})):
            result = await service.get_customers(access_token="test_token")
    ...
    @pytest.mark.asyncio
    async def test_get_customers_no_data_field(self, service):
        """Test customer retrieval when response has no data field"""
